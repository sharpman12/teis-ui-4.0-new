...
export class WebFlowGuard implements Resolve<any> {
  constructor(
  ...
  resolve(
    ...
    return this.businessMgmtService.getWebFlowById(id).pipe(map(res => {
      if (res) {
        return res;
      } else {
        this.router.navigate(['/business_management/web_flow']);
        return null;
      }
    }));
  }
