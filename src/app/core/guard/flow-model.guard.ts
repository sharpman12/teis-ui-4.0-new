...
export class FlowModelGuard implements Resolve<any> {
  constructor(
  ...
  resolve(
    ...
    return this.businessMgmtService.getFlowModelById(id).pipe(map(res => {
      if (res) {
        return res;
      } else {
        this.router.navigate(['/business_management/flow_model']);
        return null;
      }
    }));
  }
