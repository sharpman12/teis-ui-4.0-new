...
export class AdminGroupGuard implements Resolve<any> {
  ...
  resolve(
    ...
    return this.sharedService.isGroupAction ? of(this.globalDataService.getGroups()) : this.groupsService.getAllGroups(SearchCriteria).pipe(map(res => {
      if (res['userGroups']) {
        return res;
      } else {
