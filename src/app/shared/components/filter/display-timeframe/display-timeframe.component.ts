...
@Component({
  selector: 'app-display-timeframe',
  templateUrl: './display-timeframe.component.html',
  styleUrls: ['./display-timeframe.component.scss']
})
export class DisplayTimeframeComponent implements OnInit, OnChanges {

  constructor() { }
  @Input() time: string; 
  @Input() actualTimeframe: string; 
  //@Input() status:boolean; 
  dateRangeFormat: boolean;   
  displayString: string; 
  isValid: boolean;
  @Output() readonly filterTimeFrame: EventEmitter<any> = new EventEmitter();   
  @Output() readonly refresh: EventEmitter<boolean> = new EventEmitter();  
  ngOnInit() {
    
   }
  ngOnChanges() {
    if(this.time == this.actualTimeframe){
      const parsedDateValue = this.parseDate(this.time);
      this.calculateDisplayDate(this.time , parsedDateValue);
      this.filterTimeFrame.emit({display: this.displayString});
    }else{
      this.displayString = this.actualTimeframe;
      this.filterTimeFrame.emit({display: this.actualTimeframe});
    }    
  }
  onRefresh($event){
  ...
  parseDate(val: string) {
    ...
    setDisplayString(display?: string): void {
